...
namespace lwlog::details::os
{
    ...
    time_point<LocalTimePolicy>::time_point()
    {
        #ifdef _WIN32
            ::SYSTEMTIME m_now;
            ...
            year 	= static_cast<uint16_t>(m_now.wYear);
            month 	= static_cast<uint8_t>(m_now.wMonth);
            weekday = static_cast<uint8_t>(m_now.wDayOfWeek);
            day 	= static_cast<uint8_t>(m_now.wDay);
            minute 	= static_cast<uint8_t>(m_now.wMinute);
            second 	= static_cast<uint8_t>(m_now.wSecond);
            ...
            millisecond = static_cast<uint16_t>(m_now.wMilliseconds);
            microsecond = static_cast<uint32_t>((ticks_since_last_second / 10) % 1'000'000);
            nanosecond = static_cast<uint32_t>((ticks_since_last_second * 100) % 1'000'000'000);
        #else
